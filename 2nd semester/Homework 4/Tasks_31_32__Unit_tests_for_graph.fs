...
// Realisation of IGraph using the adjacency matrix
...
// Realisation of IGraph using the adjacency list
...
// Function for searching nodes which available from a given node
let availableFrom (graph : IGraph<'A>) node =
    let visited = [|for i in 0 .. graph.nodesNumber - 1 -> false|]
    dfs graph visited node 0

// Function for searching nodes which have access to a given node
